...
import { Copy } from "lucide-react"; // ← アイコン追加

export default function Home() {
  const [count, setCount] = useState<number | null>(1);

  const handleNext = () => {
    const link = "https://example.com/next"; //ここをルームのurlに変更
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium bg-blue-600 text-black rounded hover:bg-blue-700 transition"
          >
            <Copy size={16} />
          </button>
        </div>
    ...
    <main className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 animate-slide-up-fade">
      ...
      <section className="space-y-6 mt-3">
        ...
        <div className="flex justify-center">
          <Button text="完了" onClickFunc={handleNext} />
        </div>
      </section>
    </main>
  );
